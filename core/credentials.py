...
class CredentialGenerator:
    ...
    def generate_pdf(alumno):
        ...
        if len(alumno.nombre_completo) > 25:
            ...
            current_y = name_y - 6*mm
            
        # Meta Info
        c.setFont("Helvetica", 10)
        c.setFillColor(HexColor("#64748b")) # Slate 500
        
        c.drawCentredString(width/2, current_y - 5*mm, alumno.cedula)
        
        if alumno.carrera:
             c.drawCentredString(width/2, current_y - 10*mm, alumno.carrera[:35]) # Truncate if extreme
        ...
        c.setFillColor(UNEMI_BLUE)
        c.drawCentredString(width/2, 10*mm, "Presenta este código para registrar tu asistencia")
